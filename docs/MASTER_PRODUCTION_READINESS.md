...
> **Last revision**: 2026-04-26 (Sprint 58) — **Sprint 55-58 closed every code-side gap in this document.** Six tracks shipped per [`docs/architecture/sprint-55-58-code-side-readiness.md`](architecture/sprint-55-58-code-side-readiness.md): **T1** JWT in `httpOnly + Secure + SameSite=Strict` cookies + double-submit CSRF (PR #28, [ADR-0006](adr/0006-auth-via-httponly-cookies.md)) — closes **R8 / P2-1** (JWT-localStorage XSS exfil class). **T2** per-route `@route_query_budget` decorator + autouse hard-fail gate (PR #30) — closes **P2-4** (N+1 endpoint profiling); enables Engine-of-Record causality data path. **T3** pinned p50/p95 baselines for 17 endpoints + 25 % regression gate in CI (PR #31) — closes **N-6**. **T4** Langfuse activated + Transparent AI Accounting page at `/dashboard/settings/ai-usage` (PR #32, [ADR-0008](adr/0008-transparent-ai-accounting.md)) — closes **N-5** code-side; tier-aware presentation differentiates from LinkedIn/Indeed. **T5** GrowthBook-style flag system + Sentry-triggered auto-rollback + tier-aware canary delay (PR #33, [ADR-0009](adr/0009-progressive-deployment-canary.md)) — supersedes **ADR-0005** PARK decision. **T6** webhook DLQ (`webhook_event` ledger + admin replay surface) + 15-min scheduled production smoke (PR #34, [ADR-0010](adr/0010-webhook-dlq-and-prod-smoke.md)) — closes §7 verification checklist as continuous monitoring rather than gated. **Web follow-ups** (PR #35): SSR-safe feature flag SDK, AI usage page, hydration-safe relative time. Backend test count: **1,673 → 4,252+** (+2,579 across Sprints 45–58). API line coverage: **62 % → 94.69 %.** D3 9.0→9.6, D5 12.9→13.6 (R8 closed), D7 7.4→8.5 (perf baseline + query budget), D8 5.0→5.0 (already capped; +5 ADRs documented), D9 7.5→8.0 (canary + auto-rollback), D10 5.3→7.0 (Langfuse + AI accounting + DLQ). Composite **81.7 → 85.5 (+3.8)** — recomputed as the literal sum of the 10 weighted-domain scores below; supersedes earlier rolling deltas which had drifted from the table arithmetic (Gemini medium #1).
...
| Question | Answer |
| :--- | :--- |
| **Composite Score** | **85.5 / 100** (↑ 3.8 from Sprint 55-58 close; arc: 72.4 pre-ADR → 81.7 post-Sprint-44 → **85.5 post-Sprint-58**; line coverage 94.69 %; 4,252 backend tests). The new total is the literal sum of the 10 weighted-domain rows in §2 — earlier rolling deltas drifted slightly from the table; this revision re-anchors the headline to the table arithmetic. |
...
| D3 | Implementation (Tests) | 10 | 9.6 | 🟢 | **4,252 backend tests** (+2,579 since Sprint 44), 267 web, 84 mobile, ~28 E2E. **Line coverage: 94.69 %** (Sprint 47 target 80 % — far exceeded). 23 new cookie-auth + 6 sentry-rollback + 7 webhook-DLQ + AI usage tests across T1-T6. Query budget gate prevents N+1 regressions. |
...
| D8 | Documentation | 5 | 5.0 | 🟢 | **12 ADRs** (ADR-0001–0010 + this MPR + sprint-55-58 plan). 7 incident runbooks + production checklist + canary-rollback runbook. SECURITY.md §"Ignored CVEs" register. README accurate. API docs auto-generated (disabled in prod). |
...
| D10 | Observability | 10 | 7.0 | 🟡 | Structured logging + Sentry SDK integrated. `/health/ready` exposes structured `db` + `redis_detail` with TLS attestation. **Langfuse activated (T4, ADR-0008)** — per-engine trace + cost telemetry; `/dashboard/settings/ai-usage` exposes the data to users. **Webhook DLQ ledger (T6, ADR-0010)** — `webhook_event` table + admin replay surface; 15-min scheduled production smoke. `SENTRY_DSN` still empty (OPS-1) → no prod error visibility until manual setup. No external uptime monitor (OPS-6). |
| | **Total** | **100** | **85.5** | 🟢 | Literal sum of weighted-domain scores above. Arc: 72.4 → 75.0 (ADR-0001) → 76.9 (ADR-0002) → 77.4 (docs) → 78.2 (P2-3/N-7) → 79.1 (cache+R5) → 79.9 → 80.3 → 80.5 → 81.1 → 81.7 (Sprint 44 close, prior carried-over headline) → **85.5 (Sprint 58 close: T1-T6 shipped, ADR-0006/07/08/09/10, 4,252 tests, 94.69 % line coverage)**. The +3.8 delta from 81.7 is the actual table-arithmetic improvement; earlier headlines drifted vs the table by ~2 pts and this revision re-anchors. |
...
- ~~N-2 Coverage gate ≥80% on API.~~ ✅ closed Sprint 47; **94.69 % line coverage as of Sprint 58** (target was 80 %).
...
## 5. What's Built (inventory snapshot, 2026-04-26)

### Backend (`apps/api`)
- **34+ route files** (`app/api/v1/` + `admin_webhooks` + `ai_usage` + `sentry_auto_rollback`), **35+ services**, **22+ Alembic migrations**.
- **170+ test files**, **4,252 tests** passing (line coverage **94.69 %**); Ruff ✅ / mypy ✅ / pip-audit ✅ (blocking in CI); `--cov-fail-under` ratchet at floor.
- **Auth**: JWT in `httpOnly + Secure + SameSite=Strict` cookies (T1, ADR-0006) + double-submit CSRF + 30-day bearer-header overlap; OAuth (Google GIS + Microsoft MSAL + JWKS); email verification; password reset; Turnstile; refresh rotation + replay detect; fail-closed JWT blacklist.
