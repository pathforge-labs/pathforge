...
describe("isEnabled — internal_only stage", () => {
  it("returns true for internal users", () => {
    ...
    expect(isEnabled("x", flag, { ...BASE_USER, is_internal: true })).toBe(true);
  });

  it("returns false for external users", () => {
    const flag: FlagDefinition = { stage: "internal_only" };
    expect(isEnabled("x", flag, BASE_USER)).toBe(false);
  });
});
...
describe("isEnabled — percent stages", () => {
  it("user bucketing is deterministic", () => {
    ...
    const a = isEnabled("flag-x", flag, { id: "u-stable" });
    const b = isEnabled("flag-x", flag, { id: "u-stable" });
    expect(a).toBe(b);
  });
  ...
  it("5%-adopters are subset of 25%-adopters", () => {
    ...
    const enabledAt5 = new Set(
      users.filter((u) => isEnabled("flag-x", flag5, u)).map((u) => u.id),
    );
    const enabledAt25 = new Set(
      users.filter((u) => isEnabled("flag-x", flag25, u)).map((u) => u.id),
    );
  ...
  it("100% covers everyone (non-major-release path)", () => {
    ...
    for (let i = 0; i < 50; i++) {
      expect(isEnabled("flag-x", flag, { id: `u-${i}` })).toBe(true);
    }
  });
});
...
describe("isEnabled — tier-aware canary", () => {
  it("paying users are held back during 24h on major releases", () => {
    const startedAt = new Date(Date.now() - 2 * 60 * 60 * 1_000).toISOString();
    ...
    expect(isEnabled("engine-v2", flag, paying)).toBe(false);
  });

  it("paying users unblocked after 24h", () => {
    const startedAt = new Date(
      Date.now() - PAYING_USER_DELAY_MS - 60 * 1000,
    ).toISOString();
    const flag: FlagDefinition = {
      stage: "percent_100",
      major_release: true,
    ...
    expect(isEnabled("engine-v2", flag, paying)).toBe(true);
  });
  ...
  it("minor releases never gate paying users", () => {
    ...
    expect(isEnabled("patch-x", flag, paying)).toBe(true);
  });
});

describe("isEnabled — fail-closed", () => {
  it("unknown flag returns false", () => {
    expect(isEnabled("missing", null, BASE_USER)).toBe(false);
    expect(isEnabled("missing", undefined, BASE_USER)).toBe(false);
  });

  it("missing user id returns false", () => {
    const flag: FlagDefinition = { stage: "percent_100" };
    expect(isEnabled("x", flag, { id: "" })).toBe(false);
  });
});
