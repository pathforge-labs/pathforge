...
| GHSA-q3j6-qgpj-74h6 | High | `fast-uri` <=3.1.0 path-traversal (transitive via Sentry / Fastify ecosystem) | **Runtime non-attack-path** | The advisory requires attacker-controlled URI input passed to `fast-uri.parse()`. PathForge does not invoke `fast-uri` directly anywhere in application code — it ships only as a transitive dependency of upstream tooling that processes its own internal URLs (Sentry DSN parsing, Fastify route patterns), never user input. No reachable code path exists from a user request to `fast-uri` parsing. Verified by grepping `apps/web/` and the workspace root for direct imports — zero hits. | When upstream parent (Sentry SDK / Fastify) bumps to a `fast-uri >=3.1.2` resolution, remove this entry. Target: next Sentry SDK minor release. | 2026-05-10 |
| GHSA-v39h-62p7-jpjc | High | `fast-uri` <=3.1.1 host-confusion (transitive, same path as above) | **Runtime non-attack-path** | Same dependency path and reachability analysis as GHSA-q3j6-qgpj-74h6. The "host confusion" attack requires an adversary-supplied URI parsed for hostname trust decisions; we never route user input through `fast-uri` for such decisions. | Same conditions as GHSA-q3j6-qgpj-74h6. | 2026-05-10 |
| GHSA-fv7c-fp4j-7gwp | High | `@babel/plugin-transform-modules-systemjs` 7.12.0–7.29.3 (transitive via Babel toolchain) | **Build-time only / not in shipping bundle** | This Babel plugin transforms ES modules to SystemJS format. Next.js 16 builds with **SWC**, not Babel, for production output — the plugin is in the dependency graph but never invoked by our build pipeline. Verified by inspecting `apps/web/.next/` output: zero `System.register(` strings, all chunks use native ES module imports. The vulnerability is in generated SystemJS runtime code, which we do not ship. | When Babel publishes 7.29.4+ and the transitive parent upgrades, remove this entry. Target: next monthly Babel release. | 2026-05-10 |
